...
router.post('/add-player', function(request, response, next){
    ...
    //        console.log(err);
    //    });
    console.log("PRINT OUT!!!", request.body);
    var flag = false;
    Quiz.findById(request.body.id, function(err, quiz){
        console.log("QUIZ!!!", quiz);
        for(var i = 0; i<quiz.players.length; i++){
            console.log("THIS PLAYER", quiz.players[i]);
            console.log("NEW PLAYER", request.body.player);
            if(quiz.players[i] == request.body.player){
        ...
        if(flag == false){
            ...
            Quiz.findByIdAndUpdate(request.body.id, {$push:{'players': request.body.player}},
                {safe: true, upsert:true}, function(err){
                    console.log(err);
                });
            flag = false;
